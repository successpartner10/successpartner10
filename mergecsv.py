...
path="/home/sy/PycharmProjects/pythonProject/python_xl/"#with filename in column.
# Differences in the code are minimal: you simply need to replace the method read_csv()
# with read_excel() while parsing the files and to_csv() with to_excel() while writing
# the sales_records_full.xlsx back to the directory.

# 2. creates list with files to merge based on name convention
file_list = [path + f for f in os.listdir(path) if f.startswith('vfs')]
...
csv_merged.to_csv(path + '2sales_records_full.csv', index=False)